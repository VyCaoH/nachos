...
bool OpenFile::IsOpened() { return opened; }

OpenFile::OpenFile(char *name, int sector)
{ 
    hdr = new FileHeader;
    hdr->FetchFrom(sector);
    seekPosition = 0;
    opened=TRUE;
    strncpy(filename, name, MAX_LENGTH);
}

OpenFile::OpenFile(char *name, int sector, int _type)
{ 
    hdr = new FileHeader;
    hdr->FetchFrom(sector);
    seekPosition = 0;
